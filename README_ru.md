...
Мониторинг почтового ящика mail.ru на наличие уведомлений о резервном копировании с сохранением результатов в SQLite и отправкой SMTP-уведомлений.

## Как работает

1. **IMAP-мониторинг** — программа подключается к почтовому ящику mail.ru (папка `Backup`) и периодически проверяет новые письма
